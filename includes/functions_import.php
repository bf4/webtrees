...
function import_record($indirec, $update) {
	...
	case 'INDI':
		...
		$sql = "INSERT INTO ".$TBLPREFIX."individuals (i_id, i_file, i_rin, i_name, i_isdead, i_gedcom, i_letter, i_surname, i_gender) VALUES ('".$DBCONN->escapeSimple($gid)."','".$DBCONN->escapeSimple($indi["gedfile"])."','".$DBCONN->escapeSimple($indi["rin"])."','".$DBCONN->escapeSimple($names[0]['full'])."',".$DBCONN->escapeSimple($isdead).",'".$DBCONN->escapeSimple($indi["gedcom"])."','".$DBCONN->escapeSimple($initial)."','".$DBCONN->escapeSimple($surn)."', '".$DBCONN->escapeSimple($indi["gender"])."')";
		$res = dbquery($sql);
