...
//		$indirec = preg_replace(array("/".html_entity_decode("&rlm;",ENT_COMPAT,"UTF-8")."/", "/".html_entity_decode("&lrm;",ENT_COMPAT,"UTF-8")."/"), array("",""), $indirec);
		...
		$indirec = str_replace(array(chr(0xE2).chr(0x80).chr(0x8F), chr(0xE2).chr(0x80).chr(0x8E)), "", $indirec);
	}
	...
	if (!$update && !empty($fpnewged) && !(empty($indirec))) fwrite($fpnewged, trim($indirec)."\r\n");
}
...
function accept_changes($cid) {
	...
	if (isset($pgv_changes[$cid])) {
		...
		if (!empty($COMMIT_COMMAND)) check_in($logline, $GEDCOM, dirname($GEDCOMS[$GEDCOMS]['path']));
