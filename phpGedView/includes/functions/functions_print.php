	...
	if ($sub) {
		...
		if (!empty($placerec)) {
			...
			if ($map_lati && $map_long && empty($SEARCH_SPIDER)) {
				...
				$html.=' <a target="_BLANK" href="'.encode_url("http://maps.google.com/maps?q={$map_lati},{$map_long}(".PrintReady($place).")").'"><img src="images/bubble.gif" border="0" alt="Google Maps &copy;" title="Google Maps &copy;" /></a>';
