 ...
 * @version $Id: Generic_Form.php,v 1.5 2006/05/22 20:18:21 yalnifj Exp $
...
class Generic_Form extends ra_form {
	...
	function getTaskData() {
		global $TBLPREFIX, $DBCONN;
		$sql = "SELECT * FROM ".$TBLPREFIX."tasks WHERE t_id='".$DBCONN->escapeSimple($_REQUEST['taskid'])."'";
		$res = dbquery($sql);
		if ($res->numRows()>0) $row = $res->fetchRow(DB_FETCHMODE_ASSOC);
		else $row = array('t_id'=>'','t_fr_id'=>'','t_title'=>'','t_description'=>'','t_startdate'=>'','t_enddate'=>'','t_results'=>'');
		$res->free();
		return db_cleanup($row);
	}
			...
			foreach($sources as $i=>$sid) {
				if (!empty($sid)) {
					$sql = 'INSERT INTO '.$TBLPREFIX.'tasksource (ts_t_id, ts_s_id, ts_page, ts_quay, ts_date, ts_text, ts_obje) '."VALUES ('" . $DBCONN->escapeSimple($_REQUEST["taskid"]) . "', '".$DBCONN->escapeSimple($sid)."'" .
							",'".$DBCONN->escapeSimple($_REQUEST['PAGE'])."'" .
							",'".$DBCONN->escapeSimple($_REQUEST['QUAY'])."'" .
							",'".$DBCONN->escapeSimple($_REQUEST['DATE'])."'" .
							",'".$DBCONN->escapeSimple($_REQUEST['TEXT'])."'" .
							",'".$DBCONN->escapeSimple($_REQUEST['OBJE'])."')";
					$res = dbquery($sql);
				}
			}
		}
