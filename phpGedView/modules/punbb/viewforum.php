...
if (($cur_forum['post_topics'] == '' && $pun_user['g_post_topics'] == '1') || $cur_forum['post_topics'] == '1' || $is_admmod)
	$post_link = "\t\t".'<p class="postlink conr"><a href="module.php?mod=punbb&amp;pgvaction=post&amp;fid='.$id.'">'.$lang_forum['Post topic'].'</a></p>'."\n";
else
	$post_link = '';
...
if ($db->num_rows($result))
{
	while ($cur_topic = $db->fetch_assoc($result))
	{
		...
		if ($cur_topic['moved_to'] == null)
			$last_post = '<a href="module.php?mod=punbb&amp;pgvaction=viewtopic&amp;pid='.$cur_topic['last_post_id'].'#p'.$cur_topic['last_post_id'].'">'.format_time($cur_topic['last_post']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($cur_topic['last_poster']).'</span>';
		else
			$last_post = '&nbsp;';
