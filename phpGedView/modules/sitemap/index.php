...
if ($action=="sendFiles") {
    ...
    if (isset($indi_rec)) {
        $sql = "SELECT i_id,i_gedcom FROM ".$TBLPREFIX."individuals WHERE i_file='".$index."' ORDER BY 'i_id' ASC LIMIT 0,20000";
        $res = dbquery($sql);
    ...
    if (isset($fam_rec)) {
        $sql = "SELECT f_id,f_gedcom FROM ".$TBLPREFIX."families WHERE f_file='".$index."' ORDER BY 'f_id' ASC LIMIT 0,20000";
        $res = dbquery($sql);
        while ($row =& $res->fetchRow()) {
            print "   <url>\n";
