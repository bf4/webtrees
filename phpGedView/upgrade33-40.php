...
if (in_array($TBLPREFIX."dates", $tables)) {
	...
	//-- $gedarray is undefined in this context... should not be needed anway since the reimport will take care of it
...
//--create remotelinks association table
if (!in_array($TBLPREFIX."remotelinks", $tables)) {
	...
	if(!DB::isError($res)) {
		...
		$sql = "CREATE INDEX r_linkid ON ".$TBLPREFIX."remotelinks (r_linkid)";
		$res = dbquery($sql);
	}
}
}
