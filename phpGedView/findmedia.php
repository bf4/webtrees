 ...
 * @package PhpGedView
 * @subpackage Display
 * @version $Id: findmedia.php,v 1.12.2.27 2006/04/13 12:06:36 canajun2eh Exp $
 */
require("config.php");
...
if ($action=="moveto") {
	...
	if ($isadmin && $fileaccess) {
		...
		// and the thumbnail
		$tdirfrom = str_replace("$MEDIA_DIRECTORY",$MEDIA_DIRECTORY."thumbs/",$directory);
		$tdirto = str_replace("$MEDIA_DIRECTORY",$MEDIA_DIRECTORY."thumbs/",$directory).$moveto;
...
if ($action=="filter") {
	...
	if ($level>0) {
		...
		$uplink = "<a href=\"findmedia.php?embed=$embed&directory=".rawurlencode($pdir)."&thumbdir=".rawurlencode($pthumb)."&level=".($level-1).$thumbget."\">&nbsp;&nbsp;&nbsp;&lt;-- $pdir&nbsp;&nbsp;&nbsp;</a><br />\n";
	}
	...
	if (count($dirs) || $level) {
		...
		foreach ($dirs as $indexval => $dir) {
			print "\n\t\t<tr>\n\t\t\t<td class=\"list_value $TEXT_DIRECTION\">";
			print "<a href=\"findmedia.php?embed=$embed&directory=".rawurlencode($directory.$dir."/")."&thumbdir=".rawurlencode(str_replace($MEDIA_DIRECTORY, $MEDIA_DIRECTORY."thumbs/", $directory.$dir."/"))."&level=".($level+1).$thumbget."\">$dir</a><br /></td>\n\t\t</tr>";
		}
		print "\n\t</table>";
	}
	...
	if (count($images)) {
		...
		foreach ($images as $indexval => $imag) {
			...
			if ($isvalid) {
				$imgsize = findImageSize($directory.$imag);
				...
				if ($showthumb) {
					...
					if (file_exists($thumbdir.$imag)) {
						print "<a href=\"javascript:;\" onclick=\"return openImage('".preg_replace("/'/", "\'", rawurlencode($directory.$imag))."',$imgwidth, $imgheight);\"><img src=\"".filename_encode($thumbdir.$imag)."\" border=\"0\" width=\"50\"></a>\n";
					}
					else {
						print "<a href=\"findmedia.php?embed=$embed&directory=".rawurlencode($directory)."&action=thumbnail&level=$level&file=".rawurlencode($imag.$thumbget)."\">".$pgv_lang["gen_thumb"]."</a></td>";
					}
				}
...
function print_move_to_menu($dirlist,$directory, $filename) {
	...
	if ($level>0) {
		$submenu = array();
		$submenu["label"] = "<b>&nbsp;&nbsp;&nbsp;<--&nbsp;&nbsp;&nbsp;</b>";
		$submenu["link"] = "findmedia.php?embed=$embed&directory=".rawurlencode($directory)."&action=moveto&level=$level&movetodir=..&movefile=".rawurlencode($filename).$thumbget;
	...
	if ($level < $MEDIA_DIRECTORY_LEVELS) {
		foreach ($dirlist as $indexval => $dir) {
			$submenu = array();
			$submenu["label"] = $dir;
			$submenu["link"] = "findmedia.php?embed=$embed&directory=".rawurlencode($directory)."&action=moveto&level=$level&movetodir=".rawurlencode($dir)."&movefile=".rawurlencode($filename).$thumbget;
			$submenu["class"] = "submenuitem";
			$menu["items"][] = $submenu;
		}
	}
