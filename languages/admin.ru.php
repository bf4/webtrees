...
$pgv_lang["admin_gedcom"]			= "Администрирование GEDCOM";
...
$pgv_lang["day_before_month"]		= "День перед Месяцем (ДД ММ ГГГГ)";
$pgv_lang["do_not_change"]		= "Не менять";
$pgv_lang["download_gedcom"]		= "Загрузить файл GEDCOM";
$pgv_lang["download_note"]		= "ЗАМЕЧАНИЕ: Большие GEDCOM файлы могут потребовать большого времени для загрузки. Если ограничение по времени, установленное в PHP, истечёт до того как загрузка будет завершена, Вы можете получить неполную загрузку. Для проверки полноты загрузки, Вы можете проверить последнюю строку загруженного GEDCOM файла, она должна быть равна 0 TRLR. В общем импорт GEDCOM файлов может занять много времени. ";
$pgv_lang["editaccount"]			= "Позволить этому пользователю редактировать информацию своей учётной записи";
...
$pgv_lang["error_header_write"]	= "GEDCOM-файл, [#GEDCOM#], закрыт на запись. Проверьте атрибуты доступа к файлу.";
...
$pgv_lang["invalid_dates"]		= "Обнаружен неправильный формат даты, приведите все даты к формату ДД МММ ГГГГ (например, 1 JAN 2004).";
$pgv_lang["invalid_header"]		= "Обнаружены строки перед заголовком GEDCOM файла (0 HEAD). Они будут удалены при Чистке.";
...
$pgv_lang["month_before_day"]		= "Месяц перед Днем (ДД ММ ГГГГ)";
$pgv_lang["performing_validation"]	= "Производя утверждение GEDCOM, выберите необходимые опции и нажмите 'Чистка'";
...
$pgv_lang["user_create_error"]		= "Новый пользователь не создан. Попробуйте ещё раз.";
...
$pgv_lang["verify_upload_instructions"]	= "Если Вы выберете продолжить, старый файл GEDCOM будет заменён тем, который Вы выгрузили, и процесс импорта начнётся снова. Если вы выберете отмену, старый GEDCOM файл останется без изменений.";
