 ...
 * Copyright (C) 2006 Greg Roach fisharebest@users.sourceforge.net
 *
...
$DATE_GREG                            ="((@#DGREGORIAN@ )?(($DAY )?$MONTH )?$YEAR_GREG)";
$DATE_JULN                            ="(@#DJULIAN@ (($DAY ?)$MONTH ?)$YEAR(B\.C\.)?)";
...
//$NAME_TEXT                            ='\p{L}{1,120}';
//$NAME_PIECE                           ='\p{L}{1,90}';
// My version of PCRE does not support \p :-(
$NAME_TEXT                            ='.{1,120}';
$NAME_PIECE                           ='.{1,90}';
...
$TAG['PLACE_LATITUDE']                ='([NS][1-9][0-9]?(\.[0-9]+)?)';
$TAG['PLACE_LONGITUDE']               ='([EW][1-9][0-9]*(\.[0-9]+)?)';
...
// Alternative formats for objects not yet supported
...
////////////////////////////////////////////////////////////////////////////////
// Check the low-level structure of the file, line-by-line.
////////////////////////////////////////////////////////////////////////////////
$gedfile=preg_split("/${EOL}/", file_get_contents($GEDCOMS[$GEDCOM]["path"]), -1, PREG_SPLIT_NO_EMPTY);
...
foreach ($gedfile as $text) {
  // Parse each line of the file.  This regex *must* match all possible input lines.
  preg_match('/^(\s*)(\d*)(\s*)(@[^@#]+@)?(\s*)(\S*)(\s?)(.*)$/', $text, $match);
  $tag_level=$match[2];
  $xref     =$match[4];
  $tag      =$match[6];
  $tag_data =$match[8];

  // Keep a copy of tags/levels for forward checking.
  $ged_level[]=$tag_level; $ged_tag[]=$tag;

  // Make indexes for checking unused records and links between FAM and INDI.
  if ($tag_level=='0' && $xref!='') {
    $curr_xref=$xref;
    $all_xrefs[$curr_xref.$tag]=true;
  }
  if (preg_match('/^@[^@#]+@$/', $tag_data)) {
    if (isset($XREF_LINK[$tag]))
      $used_xrefs[$tag_data.$XREF_LINK[$tag]]=true;
    else
      $used_xrefs[$tag_data.$tag]=true;
    if ($tag_level=='1' && preg_match('/^(FAM[CS]|WIFE|HUSB|CHIL)$/', $tag))
      $xref_links[$curr_xref.$tag][$tag_data]=true;
  }
}
$ged_level[]=0; $ged_tag[]='DUMMY';
$num_lines=count($gedfile);

// Now scan each record in turn and report errors
$context=array('GEDCOM'); $curr_xref="";
foreach ($gedfile as $num=>$text) {
  // Parse each line of the file.  This regex *must* match all possible input lines.
  preg_match('/^(\s*)(\d*)(\s*)(@[^@#]+@)?(\s*)(\S*)(\s?)(.*)$/', $text, $match);
  $whitespace1=$match[1];
  $tag_level  =$match[2];
  ...
  if ($err_level>=$critical) { // CRITICAL CHECKS - links
    ...
    elseif (preg_match('/^(@[^#@]+@)$/', $tag_data, $match)) {
      ...
      elseif ($tag_level=='1' &&
        ...
        $err=no_link(pgv_href($XREF_LINK[$tag], $match[1]));
    }
    if ($err_level>=$error && $err=='') { // ERROR CHECKS - tags
      ...
      if ($err=='')
        for ($i=$num+1; $ged_level[$i]>$tag_level; ++$i)
          if ($ged_level[$i]==$tag_level+1)
            if (isset($count_plus_one[$ged_tag[$i]]))
              $count_plus_one[$ged_tag[$i]]++;
            else
              $count_plus_one[$ged_tag[$i]]=1;
  ...
  // If we have a valid level 1 tag, then check it for errors
  //if ($tag_level==1 && tag!="");
  ...
  if ($err!="") {
    if (isset($last_err_num)) {
      for ($i=$last_err_num+1; $i<=$last_err_num+$context_lines && $i<$num; ++$i)
        printf("%07d  %s\n", $i+1, $gedfile[$i]);
      if ($i<$num-$context_lines) {
        if ($context_lines>0) print "</pre><pre>";
        for ($i=$num-$context_lines;$i<$num;++$i)
          printf("%07d  %s\n", $i+1, $gedfile[$i]);
      } else
        for (; $i<$num; ++$i)
          printf("%07d  %s\n", $i+1, $gedfile[$i]);
...
////////////////////////////////////////////////////////////////////////////////
// If we found any errors, print the trailing context lines
////////////////////////////////////////////////////////////////////////////////
if (isset($last_err_num)) {
  for ($i=$last_err_num+1; $i<=$last_err_num+$context_lines && $i<count($gedfile); ++$i)
    printf("%07d  %s\n", $i+1, $gedfile[$i]);
  print "</pre>";
}

////////////////////////////////////////////////////////////////////////////////
// Now check the gedcom at a higher level, using the phpGedView API
////////////////////////////////////////////////////////////////////////////////
