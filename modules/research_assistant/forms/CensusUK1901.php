<?php
/**
 * phpGedView Research Assistant Tool - United Kingdom Census 1901 File
 *
...
class CensusUK1901 extends ra_form {
    ...
    function header($action, $tableAlign, $heading, $showchoose = false) {
		...
		if ($showchoose) {
			...
			$out .= '<input type="hidden" name="mod" value="research_assistant" />' .
					...
					'<input type="hidden" name="formname" value="CensusUK1901" />' .
...
//-->
        ...
        $out .= '<table id="CensusUK1901" class="list_table" dir="ltr" align="' . $tableAlign . '">';
        $out .= '<tr>';
    ...
    function simpleCitationForm($citation) {
		...
		$ct = preg_match("/RG13, Book:(.*), .*:(.*), .*:(.*), .*:(.*)/", $citation['ts_page'], $match);
		if ($ct > 0) {
			$callno = trim($match[1]);
			$folio = trim($match[2]);
			$page = trim($match[3]);
...
//        Start of Table
		...
		$out .= '</td></tr>';
        $out .= '<tr><td class="descriptionbox">Ref: RG13,   '.$pgv_lang["book"].'</td><td class="optionbox"><input name="CallNumberURL" type="text" size="27" value="'.htmlentities($callno).'"></td>';
 ...
 <tr>
  ...
  $persons = array_values($people);
    ...
    function display_form() {
        $out = $this->header("module.php?mod=research_assistant&form=CensusUK1901&action=func&func=step2&taskid=$_REQUEST[taskid]", "center", "1901 U.K. Census", true);
    ...
    function step2() {
		...
		if(empty($return))
		{
		$out = $this->header("module.php?mod=research_assistant&form=CensusUK1901&action=func&func=step3&taskid=" . $_REQUEST['taskid'], "center", "1901 UK Census");
...
<tr><td class="descriptionbox">Fact</td><td class="descriptionbox">Person</td><td class="descriptionbox">Reason</td><td class="descriptionbox">Add</td></tr>';
		...
		for($number = 0; $number < $_POST['numOfRows']; $number++)
		{
			...
			if($rows[$number]["Single"] == "Widowed")
			{
				...
				foreach($spouseFams as $sFamKey => $sFamValue)
				{
					...
					if ($deathYear) $diff = $deathYear - 1901;
						if($diff)
						{
							if($diff > 1 || $diff < 0)
							{
    ...
    function processSimpleCitation() {
		...
		$factrec = "1 CENS";
		$factrec .= "\r\n2 DATE 31 MAR 1901";
		//-- $factrec .=!empty($_POST['EnumerationDate'])?$_POST['EnumerationDate']:"31 MAR 1901";
		...
		$citation = array(
			"PAGE"=>"Ref: RG13, Book:".$_POST['CallNumberURL'].", Folio:".$_POST['folio'].", Page: ".$_POST['page'].", No: ".$_POST['schedule'],
			"QUAY"=>'',
			"DATE"=>!empty($_POST['EnumerationDate'])?$_POST['EnumerationDate']:"31 MAR 1901",
			"TEXT"=>$text,
			"OBJE"=>$_POST['OBJE'],
			"array"=>array(
			'city'=>$_POST['city'],
