 ...
 * @version $Id: Census1900.php,v 1.1 2006/04/06 20:12:35 yalnifj Exp $
 * @author Joey DiAna
 */
...
class Census1900 extends ra_form {
    ...
    function header($action, $tableAlign, $heading) {
    	...
    	$out .= '<input type="hidden" name="mod" value="research_assistant" />' .
    			...
    			'<input type="hidden" name="commonFrm" value="Census1900" />' .
...
$out = '<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 ...
 <tr>
  <td class="descriptionbox">Birth Month. 
   </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  		$out .= '<td class="optionbox"><INPUT TYPE="TEXT" SIZE="22" name = "BirthMonth'.$i.'" /></td>';
		}
 		$out .='</tr>
   <td class="descriptionbox">Birth Year. 
   </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  		$out .= '<td class="optionbox"><INPUT TYPE="TEXT" SIZE="22" name = "BirthYear'.$i.'" /></td>';
		}
 		$out .='</tr>
 <tr>
 ...
 <tr>
  <td class="descriptionbox">Year of immagration to the US.
	...
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
 		...
 		$out .='</tr> 
<tr>
  <td class="descriptionbox">Years in the US.
 </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  		$out .= '<td class="optionbox"><INPUT TYPE="TEXT" SIZE="22" name = "Years'.$i.'" /></td>';
		}
 		$out .='</tr> 
<tr>
  <td class="descriptionbox">Naturalization.
 </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  		$out .= '<td class="optionbox"><INPUT TYPE="TEXT" SIZE="22" name = "Naturalization'.$i.'" /></td>';
		}
 		$out .='</tr> 
...
<tr>
 ...
 <tr>
  <td class="descriptionbox">Months not employed.
   </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
 ...
 <td class="descriptionbox">Attended school in months. 
	...
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  		$out .= '<td class="optionbox"><INPUT TYPE="TEXT" SIZE="22" name = "School'.$i.'" /></td>';
		}
 		$out .='</tr>
  <tr>
 ...
 <tr>
  <td class="descriptionbox">Can write.
	...
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
 		...
 		$out .='</tr>
 <tr>
  <td class="descriptionbox">Can speak english. 
 </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  		$out .= '<td class="optionbox"><INPUT TYPE="CHECKBOX" name = "English'.$i.'" /></td>';
		}
 		$out .='</tr>
<tr>
 ...
 Rent:<INPUT TYPE="RADIO" value = "Rent" name = "OwnRent'.$i.'"> Own:<INPUT TYPE="RADIO" value = "Own" name = "OwnRent'.$i.'" /></td>';
	...
	$out .='</tr>
 <tr>
  <td class="descriptionbox">Owned free or mortgage.
   </td>';
  		for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  				$out .= '<td class="optionbox">
...
<span id="writeroot"></span>
    ...
    function display_form() {
        $out = $this->header("module.php?mod=research_assistant&form=Census1900&action=func&func=save&taskid=$_REQUEST[taskid]", "center", "1900 United States Federal Census");
        $out .= $this->content();
        $out .= $this->footer();
        return $out;
    }
    ...
    function save() {
		...
		while($id =& $res->fetchRow(DB_FETCHMODE_ASSOC)){
		 ...
		 if($sourid == "")
		 {
		 	$sql = "SELECT s_id FROM ".$TBLPREFIX."sources WHERE s_name LIKE '1900 US Census%'";
		 	$res = dbquery($sql);
			...
			for ($k = 0; $k < count($people); $k ++) {
				...
				$indirec .= "\r\n2 DATE ";
				$indirec .=!empty($_POST['EnumerationDate'])?$_POST['EnumerationDate']:"1900";
				$indirec .= "\r\n2 PLAC ".$_POST['city'].", ".$_POST['county'].", ".$_POST['state'].", USA";
				$indirec .= "\r\n2 @".$sourid."@";
				$indirec .= "\r\n3 PAGE Page ".$_POST['page'].", Call Number/URL ".$_POST['CallNumberURL'];
				$indirec .= "\r\n3 DATA ";
				$indirec .= "\r\n4 TEXT City ".$_POST['city'].", County ".$_POST['county'].", State ".$_POST['state'].", 1900 US Census";
				for($number = 0; $number < $_POST['numOfRows']; $number++)
				{
					...
					$indirec .= "\r\n5 CONT Birth month. ".$_POST["BirthMonth".$number];
					$indirec .= "\r\n5 CONT Birth year. ".$_POST["BirthYear".$number];
					...
					$indirec .= "\r\n5 CONT Years in the US. ".$_POST["Years".$number];
					$indirec .= "\r\n5 CONT Naturalization. ".$_POST["Naturalization".$number];
					$indirec .= "\r\n5 CONT Profession, Occupation or Trade. ".$_POST["Trade".$number];
					$indirec .= "\r\n5 CONT Months not employed. ".$_POST["Unemployed".$number];
					$indirec .= "\r\n5 CONT Attended school in months. ".$_POST["School".$number];
					$indirec .= "\r\n5 CONT Can read. ".$_POST["Read".$number];
					$indirec .= "\r\n5 CONT Can write. ".$_POST["Write".$number];
					$indirec .= "\r\n5 CONT Can speak english. ".$_POST["IEnglish".$number];
