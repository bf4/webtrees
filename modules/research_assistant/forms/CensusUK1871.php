<?php
/**
 * phpGedView Research Assistant Tool - United Kingdom Census 1871 File
 *
...
class CensusUK1871 extends ra_form {
    ...
    function header($action, $tableAlign, $heading, $showchoose = false) {
		...
		if ($showchoose) {
			...
			$out .= '<input type="hidden" name="mod" value="research_assistant" />' .
					...
					'<input type="hidden" name="formname" value="CensusUK1871" />' .
...
//-->
        ...
        $out .= '<table id="CensusUK1871" class="list_table" dir="ltr" align="' . $tableAlign . '">';
        $out .= '<tr>';
    ...
    function simpleCitationForm($citation) {
		...
		$ct = preg_match("/RG10, Book:(.*), .*:(.*), .*:(.*), .*:(.*)/", $citation['ts_page'], $match);
		if ($ct > 0) {
			$callno = trim($match[1]);
			$folio = trim($match[2]);
			$page = trim($match[3]);
...
//        Start of Table
		...
		$out .= '</td></tr>';
        $out .= '<tr><td class="descriptionbox">Ref: RG10,   '.$pgv_lang["book"].'</td><td class="optionbox"><input name="CallNumberURL" type="text" size="27" value="'.htmlentities($callno).'"></td>';
 ...
 <tr>
  ...
  $persons = array_values($people);
    ...
    function display_form() {
        $out = $this->header("module.php?mod=research_assistant&form=CensusUK1871&action=func&func=step2&taskid=$_REQUEST[taskid]", "center", "1871 U.K. Census", true);
    ...
    function step2() {
		...
		if(empty($return))
		{
		$out = $this->header("module.php?mod=research_assistant&form=CensusUK1871&action=func&func=step3&taskid=" . $_REQUEST['taskid'], "center", "1871 UK Census");
...
<tr><td class="descriptionbox">Fact</td><td class="descriptionbox">Person</td><td class="descriptionbox">Reason</td><td class="descriptionbox">Add</td></tr>';
		...
		for($number = 0; $number < $_POST['numOfRows']; $number++)
		{
			...
			if($rows[$number]["Single"] == "Widowed")
			{
				...
				foreach($spouseFams as $sFamKey => $sFamValue)
				{
					...
					if ($deathYear) $diff = $deathYear - 1871;
						if($diff)
						{
							if($diff > 1 || $diff < 0)
							{
    ...
    function processSimpleCitation() {
		...
		$factrec = "1 CENS";
		$factrec .= "\r\n2 DATE 2 APR 1871";
		//-- $factrec .=!empty($_POST['EnumerationDate'])?$_POST['EnumerationDate']:"2 APR 1871";
		...
		$rows = array();
		$text = $_POST['city'].", ".$_POST['county'].", ".$_POST['state'].", 1871 UK Census";
		for($number = 0; $number < $_POST['numOfRows']; $number++)
		{
		...
		$citation = array(
			"PAGE"=>"Ref: RG10, Book:".$_POST['CallNumberURL'].", Folio:".$_POST['folio'].", Page: ".$_POST['page'].", No: ".$_POST['schedule'],
			"QUAY"=>'',
			"DATE"=>!empty($_POST['EnumerationDate'])?$_POST['EnumerationDate']:"2 APR 1871",
			"TEXT"=>$text,
			"OBJE"=>$_POST['OBJE'],
			"array"=>array(
			'city'=>$_POST['city'],
