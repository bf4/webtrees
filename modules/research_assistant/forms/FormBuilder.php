...
if (strstr($_SERVER["SCRIPT_NAME"],"FormBuilder.php")) {
...
//        Next Table
	...
	function generate_body()
	{
		...
		$out .= preg_replace("/%FORMNAME%/",$formName,$tempForm);
		...
		for($i = 0; $i <$_REQUEST['numOfRows'];$i++)
		{
			...
			if($control == "Text")
			{
				...
				$out .= '$out .= \'<td class="optionbox"><INPUT TYPE="TEXT" SIZE="22" name="'."TextField".$i.'\'.$i.\'" value="\'.htmlentities($value).\'" /></td>\';'."\n";
				$out .= '}'."\n";
				$out .= '$out .=\'</tr>'."\n";
			}
			if($control == "ChkBox")
			...
			if($control == "RdoButton")
			{
				...
				$out .= 'for($i=0; $i<$_REQUEST[\'numOfRows\']; $i++) {'."\n";
				$out .= '$value = "";'."\n";
				$out .= 'if (isset($citation[\'ts_array\'][\'rows\'][$i][\''."RadioButton".$i.'\'])) $value = $citation[\'ts_array\'][\'rows\'][$i][\''."RadioButton".$i.'\'];'."\n";
				$out .=	'$out .= \'<td class="optionbox">'."\n";
				if(!empty($_REQUEST["SelectOptions".$i]))
				{
					$options = explode(",",$_REQUEST["SelectOptions".$i]);
				...
				{
					$value = addcslashes($value,"'");
					$out .= $value.'<input type="radio" value="'.$value.'" name="Radio\'.$i.\'"\'.($value==\''.$value.'\'?\' checked="checked"\':\'\').\' />'."\n";
					$out .= '<br />';
				}
				$out .= '\';}$out .=\'</tr>';
			}
	...
	for($i = 0; $i <$_REQUEST['numOfRows'];$i++)
		{
			...
			if($control == "RdoButton")
			{
				$out .= 'if (!empty($_POST["Radio'.$i.'".$number])) $text .= "'.$_REQUEST['Radio'.$i].': ".$_POST["Radio'.$i.'".$number];';
			}
		}
		$out .= '}';
