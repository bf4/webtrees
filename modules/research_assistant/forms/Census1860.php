    ...
    function header($action, $tableAlign, $heading, $showchoose = false) {
    	...
    	if ($showchoose) {
	    	...
	    	$out .= '<INPUT tabindex="1"  type="hidden" name="mod" value="research_assistant" />' .
	    			...
	    			'<INPUT tabindex="3"  type="hidden" name="formname" value="Census1860" />' .
...
//-->
        ...
        $out .= '<table id="Census1860" class="list_table" align="' . $tableAlign . '">';
        $out .= '<tr>';
...
//        Next Table
 ...
 <tr>
  <td class="descriptionbox">Families Numbered in the order of visitation</td>';
	...
	$out .='</tr>
 <tr>
  <td class="descriptionbox">The name of every Person whose usual place of <br /> abode on the first day of June, 1860, was in this family</td>';
  		for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
  			$value = "";
  ...
  <td class="descriptionbox">Place of Birth Naming the State, Territory, or Country</td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
		$value = "";
 		...
 		$out .='</tr>
 <tr>
  <td class="descriptionbox">Married within the year</td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
		$value = "";
	  		if (isset($citation['ts_array']['rows'][$i]['MarriedWithinYear'])) $value = $citation['ts_array']['rows'][$i]['MarriedWithinYear'];
  		$out .= '<td class="optionbox"><INPUT tabindex="'.($i*100+26).'"  TYPE="CHECKBOX" value="MarriedWithinYear" name="MarriedWithinYear'.$i.'" '.($value=='MarriedWithinYear'?' checked="checked"':'').' /></td>';
		}
 		$out .='</tr>
 <tr>
  <td class="descriptionbox">Attended School within the year 
	...
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
		$value = "";
  		...
  		$out .= '<td class="optionbox"><INPUT tabindex="'.($i*100+27).'"  TYPE="CHECKBOX" value="school" name="school'.$i.'" '.($value=='school'?' checked="checked"':'').' /></td>';
		}
 		$out .='</tr>
 <tr>
  <td class="descriptionbox">Person\'s over 20 y\'rs of age who cannot read or write
   </td>';
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
		$value = "";
	  		if (isset($citation['ts_array']['rows'][$i]['readwrite'])) $value = $citation['ts_array']['rows'][$i]['readwrite'];
  		$out .= '<td class="optionbox"><INPUT tabindex="'.($i*100+28).'"  TYPE="TEXT" SIZE="22" name = "readwrite'.$i.'" value="'.htmlentities($value).'" /></td>';
		}
 		$out .='</tr>
 <tr>
  <td class="descriptionbox">Whether deaf and dumb, blind, insane, idiotic, pauper, or convict
	...
	for($i=0; $i<$_REQUEST['numOfRows']; $i++) {
		$value = "";
  		...
  		$out .= '<td class="optionbox"><INPUT tabindex="'.($i*100+29).'"  TYPE="TEXT" SIZE="22" name = "Disablity'.$i.'" value="'.htmlentities($value).'" /></td>';
		}
 		$out .='</tr>';
        $out .= 
'<tr>
  ...
  $persons = array_values($people);
		...
		$pid = "";
			...
			$out .= '
	            <td id="peoplecell" class="optionbox">
	                   ...
	                   $out .= '</div>
	                   <INPUT tabindex="'.($i*100+30).'"  type="hidden" id="personid'.$i.'" name="personid'.$i.'" size="3" value="'.$pid.'" />';
    ...
    function display_form() {
        $out = $this->header("module.php?mod=research_assistant&form=Census1860&action=func&func=step2&taskid=".$_REQUEST['taskid'], "center", "1860 United States Federal Census", true);
    ...
    function step2() {
		...
		if(empty($return))
		{
		$out = $this->header("module.php?mod=research_assistant&form=Census1860&action=func&func=step3&taskid=" . $_REQUEST['taskid'], "center", "1860 United States Federal Census");
...
<tr><td class="descriptionbox">Fact</td><td class="descriptionbox">Person</td><td class="descriptionbox">Reason</td><td class="descriptionbox">Add</td></tr>';
		...
		for($number = 0; $number < $_POST['numOfRows']; $number++)
		{
			...
			if($rows[$number]["Single"] == "Widowed")
			{
				...
				foreach($spouseFams as $sFamKey => $sFamValue)
				{
					...
					if($spouse->getDeathYear())	$diff = $deathYear - 1860;
						if($diff)
						{
							if($diff > 1 || $diff < 0)
							{
    ...
    function processSimpleCitation() {
		...
		$factrec = "1 CENS";
		$factrec .= "\r\n2 DATE ";
		$factrec .=!empty($_POST['EnumerationDate'])?$_POST['EnumerationDate']:"1860";
		...
		$rows = array();
		$text = $_POST['city'].", ".$_POST['county'].", ".$_POST['state'].", 1860 US Census";
		for($number = 0; $number < $_POST['numOfRows']; $number++)
		{
