<?php
require_once("includes/classes/class_module.php");
require_once("modules/FamilySearch/FamilySearch.php");

// Load PGV embeding language file
global $language_settings, $LANGUAGE, $pgv_lang;
$pgv_lang['FamilySearch'] = 'FamilySearch';

class FamilySearch_PGVModule extends PGVModule {
	protected $name = 'FamilySearch';
	protected $description = 'Provides connection and synchronization with new FamilySearch.  Adds a tab to the individual page.';
	protected $version = '4.2.2';
	protected $pgvVersion = '4.2.2';
	protected $_tab = null;

	/**
	 * get the tab for this
	 * @return Tab
	 */
	public function &getTab() {
		if ($this->_tab==null) {
			$this->_tab = new FamilySearch_Tab();
			$this->_tab->setName($this->getName());
		}
		return $this->_tab;
	}

	/**
	 * does this module implement a menu
	 * should be overidden in extending classes
	 * @return boolean
	 */
	public function hasMenu() { return false; }
	/**
	 * does this module implement a tab
	 * should be overidden in extending classes
	 * @return boolean
	 */
	public function hasTab() { return true; }
	
}
?>
